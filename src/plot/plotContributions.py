...
def plotContributions(data, member):
    ...
    endMonth = utilities.switchMonth(data[0][7])
    endDay = data[0][8]
    endYear = data[0][9]
    ...
    totalMessages = 0
    for item in data:
        month = utilities.switchMonth(item[7])
        if month == 'Invalid':
            continue
        day = item[8]
        if len(day) == 1:
        ...
        if member in ("All",item[1]):
            messageCount[dateList.index(
                formattedEntry)] = messageCount[dateList.index(formattedEntry)]+1
    ...
    if member == "All":
        ...
        plt.title("A measure of {}'s contributions to the chat".format(
            member), fontsize=18)
    ...
    plt.xticks([0, len(messageCount)], [startDate, endDate])
